...
fn main() {
    ...
    let app = App::new("RePair")
        ...
        .arg(Arg::from_usage("-b --blocksize [INTEGER] 'Sets block size of PGE'").default_value("6"))
    ...
    if matches.is_present("c") {
        ...
        let blocksize = match matches.value_of("blocksize") {Some(x) => (*x).parse::<u32>().unwrap(), None => 6,};
